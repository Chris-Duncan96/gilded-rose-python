from item import Item, Sulfuras, BackstagePass, AgedBrie
...
class GildedRoseTest(TestCase):
    ...
    def test_quality_goes_up_for_improving_products(self):
        self.items.append(AgedBrie(20, 30))
        self.items.append(BackstagePass(20, 30))
        gilded_rose.update_quality(self.items)
        expected = [
    ...
    def test_quality_goes_up_by_two_for_improving_products_with_10_days_or_less_left(self):
        self.items.append(AgedBrie(10, 34))
        self.items.append(BackstagePass(8, 30))
        gilded_rose.update_quality(self.items)
        expected = [
    ...
    def test_quality_goes_up_by_three_for_improving_products_with_5_days_or_less_left(self):
        self.items.append(AgedBrie(4, 11))
        self.items.append(BackstagePass(5, 15))
        gilded_rose.update_quality(self.items)
        expected = [
    ...
    def test_backstage_passes_and_brie_go_to_quality_zero_after_sell_by(self):
        self.items.append(AgedBrie(0, 20))
        self.items.append(BackstagePass(0, 20))
        gilded_rose.update_quality(self.items)
        expected = [
            {'sell_in': -1, 'quality': 0},
            {'sell_in': -1, 'quality': 0},
    ...
    def test_quality_does_not_increase_past_50(self):
        self.items.append(AgedBrie(4, 49))
        gilded_rose.update_quality(self.items)
